...
namespace SpatialTest
{
    ...
    float HierarchicalGrid_omp::s_f32ObjectCellRatio        = 4.0f;
    
    // [rad] Cells on next level are twice the size
    float HierarchicalGrid_omp::s_f32CellGrowth             = 4.0f;
    
    // [rad] Epsilon value for checks
    float HierarchicalGrid_omp::s_f32Epsilon                = 0.0005f;
    
    
    
    //--
    unsigned int
    HierarchicalGrid_omp::ComputeHashValue(int i32HashBuckets, int i32X, int i32Y, int i32Z, int i32Level)
    {
    ...
    //--
    HierarchicalGrid_omp::HierarchicalGrid_omp(int i32HashBuckets) :
        ISpatialStructure(),
        m_i32FrameCount(0),
...
#pragma omp parallel
{
        ...
        for(int i32Index = 0; i32Index < m_i32HashBuckets; i32Index++)
        {
            ...
            pBucket = new HierarchicalGridHashBucket_omp();
...
#pragma omp parallel
{
        ...
        for(iter_object = m_vecObjects.begin(); iter_object != m_vecObjects.end(); iter_object++)
        {
            ...
            pBucket = static_cast<HierarchicalGridHashBucket_omp*>(pObject->VGetCell());
    ...
    //--
    HierarchicalGridHashBucket_omp::HierarchicalGridHashBucket_omp() :
        ISpatialCell(),
        m_i32LastFrame(0),
        m_i32ObjectCount(0),
        m_pObjects(NULL)
    ...
    inline 
    int
    HierarchicalGridHashBucket_omp::GetLastFrame() const
    {
    ...
    //--
    void
    HierarchicalGridHashBucket_omp::InsertObject(ISpatialObject* pObject)
    {
    ...
    //--
    void
    HierarchicalGridHashBucket_omp::RemoveObject(ISpatialObject* pObject)
    {
    ...
    //--
    void
    HierarchicalGridHashBucket_omp::CheckCollisions(int i32LastFrame, ISpatialObject* pObject)
    {        
