...
{
    //--
    int KDTree_omp::s_i32MaxDepth       = 12;




    //--
    KDTree_omp::KDTree_omp(const Vector3& refCenter, float f32HalfWidth) :
        ISpatialStructure(),
        m_vec3Center(refCenter),
        m_f32HalfWidth(f32HalfWidth)
    {
        m_pRootNode = new KDTreeNode_omp(NULL, 0);
    }



    //--
    KDTree_omp::~KDTree_omp()
    {
        if(m_pRootNode)
        {
            delete(m_pRootNode);
    ...
    inline
    void
    KDTree_omp::Preallocate(int i32Depth)
    {
    ...
    //--
    void
    KDTree_omp::VAddObjects(const std::vector<ISpatialObject*>& refObjects)
    {
    ...
    KDTree_omp::VUpdate()
    {
        ...
        {
            pIter = (*iter_object);
            pNode = static_cast<KDTreeNode_omp*>(pIter->VGetCell());
    ...
    //--
    int KDTreeNode_omp::s_i32BinCount                               = 32;

    float KDTreeNode_omp::s_f32AcceptableRatioLower                 = 0.4f;
    float KDTreeNode_omp::s_f32AcceptableRatioUpper                 = 0.6f;

    std::vector<std::pair<int, int> > KDTreeNode_omp::s_vecBins     = std::vector<std::pair<int, int> >(s_i32BinCount, std::pair<int, int>());
    std::vector<std::pair<int, int> > KDTreeNode_omp::s_vecSums     = std::vector<std::pair<int, int> >(s_i32BinCount, std::pair<int, int>());
    ...
    //--
    void
    KDTreeNode_omp::Preallocate(int i32Depth)
    {
        if(i32Depth)
        {
            m_pChildLeft = new KDTreeNode_omp(this, (m_i32SplitPane + 1) % 3);
            m_pChildLeft->Preallocate(i32Depth - 1);

            m_pChildRight = new KDTreeNode_omp(this, (m_i32SplitPane + 1) % 3);
    ...
    inline
    int
    KDTreeNode_omp::CheckContains(ISpatialObject* pObject)
    {
    ...
    inline
    int
    KDTreeNode_omp::CheckStraddle(ISpatialObject* pObject)
    {
    ...
    //--
    void
    KDTreeNode_omp::RemoveObject(ISpatialObject* pObject)
    {
        ...
        KDTreeNode_omp* pNode = m_pParent;
    ...
    //--
    void
    KDTreeNode_omp::AddObject(ISpatialObject* pObject)
    {
        Vector3 vec3Center = pObject->VGetPosition();
        float f32Radius = pObject->VGetRadius();
    ...
    //--
    void
    KDTreeNode_omp::CheckCollisions(ISpatialObject* pObject)
    {
        Vector3 vec3Center = pObject->VGetPosition();
        float f32Radius = pObject->VGetRadius();
    ...
    //--
    void
    KDTreeNode_omp::Construct(ISpatialObject* pObjectList, int i32ObjectCount,
