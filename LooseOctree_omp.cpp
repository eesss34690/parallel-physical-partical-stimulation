...
{
    //--
    int LooseOctree_omp::s_i32MaxDepth                      = 5;
    float LooseOctree_omp::s_f32MaxObjectNodeRatio          = 8.0f;
    
    
    //--
    LooseOctree_omp::LooseOctree_omp(const Vector3& refCenter, float f32HalfWidth, int i32Rebuild) :
        m_i32Rebuild(i32Rebuild),
        m_f32HalfWidth(f32HalfWidth)
    {
        // [rad] Create root node (0 depth)
        m_pRootNode = new LooseOctreeNode_omp(NULL, refCenter, f32HalfWidth, 0);
    }
    
    
    
    //--
    LooseOctree_omp::~LooseOctree_omp()
    {
        if(m_pRootNode)
        {
            delete(m_pRootNode);
    ...
    inline
    void
    LooseOctree_omp::Preallocate(int i32Depth)
    {
    ...
    {   
        ...
        {      
            ...
            for(iter_object = m_vecObjects.begin(); iter_object != m_vecObjects.end(); iter_object++)
            {
                pNode = static_cast<LooseOctreeNode_omp*>((*iter_object)->VGetCell());
    ...
    //--
    void
    LooseOctree_omp::VAddObjects(const std::vector<ISpatialObject*>& refObjects)
    {        
    ...
    //--
    LooseOctreeNode_omp::LooseOctreeNode_omp(LooseOctreeNode_omp* pParent, const Vector3& refCenter, float f32HalfWidth, int i32Depth) :
    ...
    //--
    LooseOctreeNode_omp::~LooseOctreeNode_omp()
    {
        for(int i = 0; i < 8; i++)
        {
            if(m_pChildren[i])
    ...
    //--
    void
    LooseOctreeNode_omp::CheckCollisions(ISpatialObject* pObject)
    {        
    ...
    inline
    int
    LooseOctreeNode_omp::CheckBoundaries(ISpatialObject* pObject)
    {
        /*
        int i32Index = 0;
    ...
    inline
    int
    LooseOctreeNode_omp::CheckContains(ISpatialObject* pObject)
    {
        int i32Index = 0;
    ...
    //--
    int
    LooseOctreeNode_omp::CheckContainsNonLoose(ISpatialObject* pObject)
    {
        int i32Index = 0;
    ...
    //--
    void
    LooseOctreeNode_omp::AddObject(ISpatialObject* pObject)
    {
        int i32Index = 0;
        int i32Position = 0;
        int i32Straddle = 0;
    ...
    inline
    bool
    LooseOctreeNode_omp::IsLeaf() const
    {
    ...
    //--
    void
    LooseOctreeNode_omp::Preallocate(int i32Depth)
    {
        ...
        if(i32Depth && IsLeaf())
        {
            ...
            for(int i = 0; i < 8; i++)
            {
                ...
                // [rad] Allocate node
                m_pChildren[i] = new LooseOctreeNode_omp(this, vec3NewCenter, f32Step, m_i32Depth + 1);
